...
async def get_watchlist(
    ...
    _user: dict = Depends(get_current_user),  # any authenticated user can read
):
...
async def add_to_watchlist(
    ...
    db: AsyncSession = Depends(get_db),
    _user: dict = Depends(require_manager),  # only manager/admin can set status
):
    result = await db.execute(
        select(TenderWatchlist).where(TenderWatchlist.kd_tender == item_in.kd_tender)
    )
...
async def update_watchlist(
    ...
    db: AsyncSession = Depends(get_db),
    _user: dict = Depends(require_manager),  # only manager/admin can update status
):
